...
void EnvelopeLibraryEntry::addPoint(const QPointF& point)
{
    addNode(point.x(), point.y());
}

void EnvelopeLibraryEntry::removePoint(int index)
{
    removeNode(index);
}

void EnvelopeLibraryEntry::movePoint(int index, const QPointF& point)
{
    moveNode(index, point.x(), point.y());
}

void EnvelopeLibraryEntry::insertPoint(int index, const QPointF& point)
{
    // First add the node at the end
    addNode(point.x(), point.y());
    
    // Then move it to the desired position
    // TODO: Implement proper insertion at index
}

QPointF EnvelopeLibraryEntry::getPoint(int index) const
{
    const EnvelopeNode* node = getNode(index);
    return node ? QPointF(node->x, node->y) : QPointF();
}

int EnvelopeLibraryEntry::findPoint(const QPointF& point) const
{
    return findClosestNode(point.x(), point.y(), 0.1); // 0.1 is the threshold
}

int EnvelopeLibraryEntry::getPointCount() const
{
    int count = 0;
    EnvelopeNode* current = firstNode;
    while (current) {
